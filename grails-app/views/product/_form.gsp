...
<div class="fieldcontain ${hasErrors(bean: productInstance, field: 'productManager', 'error')} ">
	...
	<g:select id="productManager" name="productManager.id" from="${org.sormer.Expert.list()}" optionKey="id" value="${productInstance?.productManager?.id}" class="many-to-one" noSelection="['null': '']"class="form-control"/>
...
<div class="fieldcontain ${hasErrors(bean: productInstance, field: 'releases', 'error')} ">
	...
	<g:select name="releases" from="${org.sormer.Release.list()}" multiple="multiple" optionKey="id" size="5" value="${productInstance?.releases*.id}" class="many-to-many"class="form-control"/>
